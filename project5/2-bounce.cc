...
int main(int argc, char *argv[]) {
  ...
  if ((rval = getaddrinfo(NULL, argv[2], &hints, &results)) != 0) { // error
...
if ((rval = getaddrinfo(argv[1], argv[2], &hints, &results)) != 0) { // error  the null needs to be fromaddress
  std::cerr << "Error getting the destination address: " << gai_strerror(rval) << std::endl;
  return 2;
}
...
if(sendto(sock, buf, strlen(buf), 0, ptr->ai_addr, ptr->ai_addrlen) == -1) {
